...
from models import Informer, Autoformer, Transformer, DLinear, Linear, NLinear, PatchTST, CATS, FITS, iTransformer, TimesNet, TiDE, FormerBaseline, Autoregressive, ARM
            ...
            for i, data in enumerate(vali_loader):
                ...
                if self.args.use_amp:
                    with torch.autocast(device_type='cuda', dtype=torch.bfloat16):
                        ...
                        elif 'ARM' in self.args.model or 'TSP' in self.args.model:
                            x = torch.cat([batch_x, torch.zeros_like(batch_y[:, -pred_len:, :], device=batch_x.device)], dim=1)
                            x_mark = torch.cat([batch_x_mark, batch_y_mark[:, -pred_len:, :]], dim=1)
                            outputs = self.model(x, x_mark=x_mark, train=False)
                ...
                if self.args.use_amp:
                    with torch.autocast(device_type='cuda', dtype=torch.bfloat16):
                        if 'CATS' in self.args.model or 'Autoregressive' in self.args.model:
                            ...
                            x = torch.cat([batch_x, batch_y[:, -pred_len:, :]], dim=1)
                            x_mark = torch.cat([batch_x_mark, batch_y_mark[:, -pred_len:, :]], dim=1)
                            outputs = self.model(x, x_mark=x_mark, drop_mask=drop_mask, train=True)
                        elif 'Linear' in self.args.model or 'TST' in self.args.model:
                            outputs = self.model(batch_x, batch_x_mark, dec_inp, batch_y_mark)
                        else:
                            if self.args.output_attention:
